...
_azure_client: openai.AzureOpenAI | None = None


def _get_client() -> openai.OpenAI:
    """Return a cached OpenAI client, initializing on first use."""
...
def _get_azure_client() -> openai.AzureOpenAI:
    """Return a cached AzureOpenAI client, initializing on first use."""
    global _azure_client
    if _azure_client is None:
        _azure_client = openai.AzureOpenAI(
            api_key=CFG.openai_api_key,
            api_version=CFG.openai_api_version,
            azure_endpoint=CFG.openai_api_base,
        )
        ...
        try:
            if CFG.use_azure:
                response = _get_azure_client().chat.completions.create(
                    model=CFG.get_azure_deployment_id_for_model(model),
                    messages=messages,
            ...
            if CFG.debug_mode:
                print(Fore.RED + "Error: Reached rate limit, retrying..." + Fore.RESET)
            if not warned_user:
                logger.double_check(
...
def create_embedding_with_ada(text) -> list:
    ...
    for attempt in range(num_retries):
        ...
        try:
            if CFG.use_azure:
                return _get_azure_client().embeddings.create(
                    input=[text],
                    model=CFG.get_azure_deployment_id_for_model("text-embedding-ada-002"),
                ).data[0].embedding
            else:
